...
namespace {
  ...
  SDL_FRect calculate_text_rect(SDL_FRect const& area, SDL_Texture* texture)
  {
    ...
    float const w = std::min(text_w, area.w-LineInput::MIN_WIDTH-8);
    float const h = std::min(text_h, area.h-LineInput::MIN_HEIGHT-8);
...
LineInput::LineInput(float const x, float const y, float const w, float const h, std::string value)
...
SDL_FRect LineInput::get_bounding_box() const
{
  return {x_, y_, w_, h_};
}
...
void LineInput::render(SDLRenderer& renderer)
{
  ...
  SDL_FRect const target_rects[9] = {
      {.x = x_, .y = y_, .w = 8, .h = 8},
  ...
  for (int n = 0; n<9; ++n)
    SDL_RenderTexture(renderer, background, ::TEXTURE_RECTS+n, target_rects+n);

  auto const text = TTF_RenderText_Solid(font_, value_.c_str(), value_.length(), {0, 0, 0, SDL_ALPHA_OPAQUE});
  ...
  SDL_FRect const text_texture_rect{
      .x = 0, .y = 0, .w = text_rect.w, .h = text_rect.h,
  };
  SDL_RenderTexture(renderer, text_ure, &text_texture_rect, &text_rect);
...
void LineInput::set_current_window(SDL_Window* window)
{
  window_ = window;
}
